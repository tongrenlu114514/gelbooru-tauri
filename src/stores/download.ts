...
export const useDownloadStore = defineStore('download', () => {
  ...
  // Initialize store: load tasks from backend and set up listeners
  async function init() {
    if (initialized) return;
    initialized = true;

    // Load existing tasks from backend (restores from DB)
    await restoreTasks();
  ...
  async function restoreTasks() {
    try {
      const result = await invoke<DownloadTask[]>('restore_download_tasks');
      tasks.value = result;
    } catch (error) {

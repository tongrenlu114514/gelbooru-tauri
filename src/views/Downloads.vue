  ...
  },
  {
    ...
    render(row) {
      const statusMap: Record<
        string,
        { type: 'default' | 'info' | 'success' | 'warning' | 'error'; label: string }
      > = {
    ...
    render(row) {
      ...
      if (row.status === 'downloading') {
        buttons.push(
          h(
            NButton,
            { size: 'small', onClick: () => downloadStore.pauseDownload(row.id) },
            { default: () => '暂停' }
          )
        );
      }
      ...
      if (row.status === 'paused') {
        buttons.push(
          h(
            NButton,
            { size: 'small', type: 'primary', onClick: () => downloadStore.resumeDownload(row.id) },
            { default: () => '继续' }
          )
        );
      }
      ...
      if (row.status === 'completed') {
        buttons.push(
          h(
            NButton,
            { size: 'small', type: 'success', onClick: () => downloadStore.openFile(row.savePath) },
            { default: () => '打开' }
          )
        );
      }
